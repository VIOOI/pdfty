	...
	return <Show when={files.value.length > 0 && !isReadStream() && convertFile.value.length === 0}>
		<EditorCards>
			<For each={files.value}>{ item =>
				...
				>
					<div class="icon" innerHTML={icons()[`${params().tool}_to_pdf`]} ></div>
				</Card>
			}</For>
			<RightMenuComponent
...
const DownloadFiles = ({ convertFile, isReadStream }) => (
	<Show when={!isReadStream() && convertFile.value.length > 0}>
		<LoadingPage>
			...
			<Button 
				type="primary"
				rounded="md"
				onClick={() => downloadFile({ files: convertFile, format: "pdf" })}
			>
        Download
			</Button>
			<p class="drop">or Drop</p>
